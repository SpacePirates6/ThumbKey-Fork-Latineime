...
fun performKeyAction(
    ...
    when (action) {
        is KeyAction.CommitText -> {
            ...
            if (text.length == 1 && PredictionEngine.isWordSeparator(text[0])) {
                // Finish composing span before word separator
                pe.finishComposing(ic)

                val didAutocorrect = try {
                    pe.onWordSeparator(ime, text)
                } catch (e: Exception) {
                    Log.w(TAG, "Prediction onWordSeparator failed", e)
                    false
                }
                if (!didAutocorrect) {
                ...
                try {
                    pe.onCharCommitted(text[0], ime)
                    pe.onKeyAction(ime)
                } catch (e: Exception) {
                    Log.w(TAG, "Prediction onKeyAction failed", e)
                }
            } else {
                // Commit phantom space before the next real character
                if (text.length == 1 && !PredictionEngine.isWordSeparator(text[0])) {
                    if (pe.consumePhantomSpace()) {
                        ic?.commitText(" ", 1)
                    }
                }

                // Use composing text for letter characters (underline preview)
                if (text.length == 1 && text[0].isLetter() && pe.composingEnabled && !pe.suppressPredictions) {
                    pe.onCharCommitted(text[0], ime)
                    pe.updateComposingText(ic)
                } else {
                    pe.finishComposing(ic)
                    keyboardSettings.textProcessor?.handleCommitText(ime, text)
                        ?: ic.commitText(text, 1)
        ...
        is KeyAction.SendEvent -> {
            ...
            try { ime.predictionEngine.clearSuggestions() } catch (e: Exception) {
                Log.w(TAG, "Prediction clearSuggestions failed", e)
            }
            onKeyEvent()
        }
        ...
        is KeyAction.DeleteKeyAction -> {
            ...
            val handledInComposing = try {
                ...
                Log.w(TAG, "Prediction handleBackspaceWhileComposing failed", e)
                false
            }
            ...
            if (!handledInComposing) {
                ...
                val didUndo = try {
                    ...
                    Log.w(TAG, "Prediction onBackspace failed", e)
                    false
                }
                if (!didUndo) {
                ...
                try {
                    ...
                    Log.w(TAG, "Prediction onCharDeleted failed", e)
                }
            }
            try {
                pe.onKeyAction(ime)
        ...
        is KeyAction.DeleteWordBeforeCursor -> {
            ...
            if (deletedText.isNotEmpty()) {
                ...
                val cy = ime.cursorScreenY.value
                if (!cx.isNaN() && !cy.isNaN()) {
                    ime.emitPdcShootdown(deletedText, cx, cy)
                }
            }
            try { ime.predictionEngine.clearSuggestions() } catch (e: Exception) {
                Log.w(TAG, "Prediction clearSuggestions failed", e)
        ...
        KeyAction.AcceptSuggestion -> {
            val pe = ime.predictionEngine
            val sugs = pe.suggestions
            if (sugs.size >= 2) {
                pe.onSuggestionSelected(1, ime)
            } else if (sugs.size == 1) {
                pe.onSuggestionSelected(0, ime)
            }
        }
