...
struct LanguageModelState {
    ...
    std::vector<std::pair<float, token_sequence>> Sample(DecodeResult decodeResult, int n_results, WordCapitalizeMode capitals, const std::vector<banned_sequence> &banned_sequences) {
        ...
        for(const auto &seq : sequences) {
            if(seq.second.tokens.front() > 48 || std::abs(seq.first - sequences[0].first) > EPS) {
                is_bugged = false;
                break;
            }
        }
...
namespace latinime {
    ...
    static void xlm_LanguageModel_getSuggestions(JNIEnv *env, jclass clazz,
        ...
        // Null-guard: inComposeX/inComposeY may be null when called in typing mode
        // without coordinate data (e.g. from the Kotlin bridge passing empty arrays)
        size_t inputSize = (inComposeX != nullptr) ? env->GetArrayLength(inComposeX) : 0;
        ...
        // Null-guard: bannedWordsArray may be null when no words are banned
        size_t numBannedWords = (bannedWordsArray != nullptr) ? env->GetArrayLength(bannedWordsArray) : 0;
        ...
        int xCoordinates[inputSize > 0 ? inputSize : 1];
        int yCoordinates[inputSize > 0 ? inputSize : 1];
        if(inputSize > 0 && inComposeX != nullptr && inComposeY != nullptr) {
