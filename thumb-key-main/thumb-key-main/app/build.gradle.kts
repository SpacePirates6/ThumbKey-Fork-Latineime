...
android {
    ...
    defaultConfig {
        ...
        // Configure C++ build for FUTO LanguageModel (LLM prediction)
        externalNativeBuild {
            cmake {
                cppFlags("-std=c++17")
            }
    ...
    // Configure external native build for CMake (FUTO LLM native code)
    externalNativeBuild {
        cmake {
            path = file("src/main/cpp/jni/CMakeLists.txt")
            version = "3.22.1"
        }
    }
    ...
    buildTypes {
        ...
        debug {
            ...
            // Only build for arm64-v8a in debug — all modern Android phones use this ABI.
            // This cuts native C++ compile time by ~75% (4 ABIs → 1).
            ndk {
    ...
    // Don't compress model files — they need to be read as raw binary data
    androidResources {
...
dependencies {
    ...
    // Coroutines (needed for LLM prediction bridge)
    implementation("org.jetbrains.kotlinx:kotlinx-coroutines-android:1.7.3")

    // WorkManager (auto-scheduled background training)
